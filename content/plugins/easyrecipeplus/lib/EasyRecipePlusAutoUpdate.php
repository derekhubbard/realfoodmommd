...
/**
 * Class EasyRecipePlusAutoUpdate
 *
...
class EasyRecipePlusAutoUpdate {
    ...
    private $slug = 'easyrecipeplus/easyrecipeplus';
    private $version;
    private $updateURL;
    private $licenseKey;
    ...
    function __construct($version, $updateURL, $licenseKey = '') {
        ...
        /**
         * This action gets fired when when we call update.php?action=update-easyrecipeplus (which is what the plugin site check version api should return as the "update" link)
         * Need to concatenate the strings below because the phing token replacement croaks on 4 underscores
         */
        add_action('update-custom_' . 'easyrecipeplus-update', array($this, 'forceUpdate'));
    ...
    public function checkLicense($value, $package) {
        ...
        if (strpos($package, 'easyrecipeplus.zip') !== false && empty($this->licenseKey)) {
            $value = new WP_Error('noeasyrecipepluslicense', __('You must enter your license key to update'));
        }
        return $value;
    }
